...
export const removeImageBackground = async (req, res) => {
  try {
    ...
    if (plan !== 'premium') {
      return res.json({
        success: false,
        message: "This feature is only available for the premium subscriptions."
      });
    }
...
export const removeImageObject= async(req, res)=>{
    try{
        ...
        if(plan!=='premium'){
            return res.json({success: false, message: "This feature is only available for the premium subscriptions."})
        }
...
export const resumeReview= async(req, res)=>{
   ...
   if(resume.size > 5*1024*1024){
    return res.json({success: false, message: "Resume file size exceeds the allowed file size(i.e. 5MB"})
   }
  ...
  const prompt= `Review the following resume and provide constructive feedback on its strengths, weaknesses, and areas of improvement
                according to ATS. Resume conten:- \n\n${pdfData.text}`
